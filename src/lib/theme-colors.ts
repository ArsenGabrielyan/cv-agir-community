...
export default function setGlobalColorTheme(
  ...
  if (!theme) {
    console.error(`Theme "${color}" not found! Available themes:`, Object.keys(themes));
    return;
  }
  const modeStyles = theme[themeMode] as {
    [key: string]: string
  };
  if (!modeStyles) {
    console.error(`Theme mode "${themeMode}" not found in "${color}"!`);
    return;
  }
  for (const key in modeStyles) {
